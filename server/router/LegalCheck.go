...
func (br *LegalCheckSaltRouter) Handle(req ziface.IRequest) {
	...
	if len(req.GetData()) != 0 {
		...
		req.GetConnection().SendMsg(0x103, errSend)
		return
	}
	// 重置随机数种子，后获取一个随机1000~2000整数
	rand.Seed(time.Now().Unix())
	salt := rand.Intn(1000) + 1000
	var saltSend protos.LegalCheckSalt = protos.LegalCheckSalt{Salt: strconv.Itoa(salt)}
...
func (br *SaltCheck) Handle(req ziface.IRequest) {
	...
	if err != nil {
		...
		return
	}else if serverCalculated == salt.GetResult() {
		res.Status = true
		res.Error = ""
